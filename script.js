...
async function initializeAppLogic(initialUser) {
    ...
    function loadAndDisplayData(data) {
        ...
        const oldStandaloneMain = data.standaloneMainQuests || [];
        const oldGeneralGroups = data.generalTaskGroups || [];

        dailyItems = data.dailyItems || [...oldStandaloneDaily, ...oldDailyGroups];
    ...
    const renderDailyItems = () => {
        ...
        dailyItems.forEach(item => {
            let el;
            if (item.isShared) {
                // This is a placeholder for a shared item
                if (item.sharedQuestId) {
                    const sharedQuest = sharedQuests.find(sq => sq.id === item.sharedQuestId);
                    if (sharedQuest) {
                        el = createTaskElement(sharedQuest, 'shared');
                    } else {
                        // Orphaned shared quest placeholder, revert to local task for cleanup
                        // IMPORTANT: Directly modify the item in the array for persistence
                        item.isShared = false;
                        delete item.sharedQuestId;
                        el = createTaskElement(item, 'daily');
                        // Add a class to indicate it's an orphaned placeholder for styling/cleanup
                        el.classList.add('is-shared-task', 'orphaned-placeholder');
                    }
                } else if (item.sharedGroupId) {
                    // Daily items shouldn't be shared groups, but handle defensively
                    const sharedGroup = sharedGroups.find(sg => sg.id === item.sharedGroupId);
                    if (sharedGroup) {
                        el = createSharedGroupElement(sharedGroup); // Render as shared group
                    } else {
                        // Orphaned shared group placeholder, revert to local group
                        // IMPORTANT: Directly modify the item in the array for persistence
                        item.isShared = false;
                        delete item.sharedGroupId;
                        el = createGroupElement(item);
                        el.classList.add('is-shared-task', 'orphaned-placeholder');
                    }
                }
            } else if (item.tasks) {
            ...
            if (item.isShared) {
                ...
                if (item.sharedQuestId) {
                    ...
                    if (sharedQuest) {
                        ...
                        item.isShared = false;
                        delete item.sharedQuestId;
                        el = createTaskElement(item, item.tasks ? 'group' : 'standalone');
                        el.classList.add('is-shared-task', 'orphaned-placeholder');
                    }
                } else if (item.sharedGroupId) {
                    const sharedGroup = sharedGroups.find(sg => sg.id === item.sharedGroupId);
                    if (sharedGroup) {
                        el = createSharedGroupElement(sharedGroup);
                    } else {
                        // Orphaned shared group placeholder, revert to local group
                        // IMPORTANT: Directly modify the item in the array for persistence
                        item.isShared = false;
                ...
                // It's a local standalone task
                el = createTaskElement(item, 'standalone');
            }
            if (el) {
    ...
    const createSharedTaskElement = (task, group) => {
        ...
        const friendUsername = isOwner ? group.friendUsername : group.ownerUsername;
        const friendStatusIndicatorHTML = `<div class="status-indicator ${friendIsCompleted ? 'completed' : ''}" title="${friendUsername}'s status"></div>`;
    ...
    /**
     * Builds the common action buttons for a task.
    ...
    /**
     * Populates a task list item with content for a shared quest.
     * @param {HTMLElement} li - The list item element.
     * @param {object} task - The shared quest data object.
     */
    const buildSharedTaskContent = (li, task) => {
        ...
        if (task.isFinishing) li.classList.add('shared-quest-finished');
        if (allCompleted) li.classList.add('all-completed');
        ...
        li.innerHTML = `
            <div class="multi-select-checkbox"></div><div class="completion-indicator"></div>
            <div class="task-content">
                <div class="task-title-and-subtitle">
                    <span class="task-text">${task.text}</span>
                    <span class="shared-with-tag">${friendStatusIndicatorHTML} ${otherPlayerUsername}</span>
                </div>
            </div>
    ...
    const deleteGroup = (id) => {
        ...
        showConfirm(`Delete "${name}"?`, 'All tasks will be deleted.', () => {
            if (group.type === 'daily') {
                ...
                mainItems = mainItems.filter(item => item.id !== id);
            }
            renderAllLists();
            saveState();
            ...
            if (e.target.closest('.group-actions') || e.target.closest('.cancel-share-group-btn') || e.target.closest('.cleanup-orphan-group-btn')) {
                ...
                if (isCleanupClick) {
                    ...
                    showConfirm(
                        ...
                        () => { // eslint-disable-line
                            // Revert local state
                            const localGroup = mainItems.find(g => g.tasks && g.id === groupId); // Changed from generalTaskGroups
                            if (localGroup) {
    ...
    function initSortable() {
        ...
        function onSortEnd(evt) {
            ...
            if (fromListEl === toListEl && (fromListEl.id === 'daily-combined-list-container' || fromListEl.id === 'main-combined-list-container')) {
                ...
                const [item] = listArray.splice(evt.oldIndex, 1);
                listArray.splice(evt.newIndex, 0, item);
                saveState();
                return;
            }

            // Case 2: Moving tasks around (in/out/between groups)
            if (!itemId || !itemEl.classList.contains('task-item')) return;
            ...
            if (toListEl.id === 'daily-combined-list-container') destListArray = dailyItems;
                ...
                const { group } = findTaskAndContext(toListEl.dataset.groupId);
        ...
        const commonOptions = (groupName) => ({
            ...
            group: {
                ...
                pull: true,
                put: (to, from, dragEl) => {
                    // Prevent groups from being dropped inside other groups' task lists.
                    if (to.el.classList.contains('task-list-group') && dragEl.classList.contains('main-quest-group')) {
                        return false;
                    }
                    return true;
                }
            },
            handle: '.main-quest-group-header, .task-item', // Allow dragging both groups and tasks
        ...
        // Make the inner group lists sortable and connect them
    ...
    async function removeFriend(e) {
        ...
        showConfirm("Remove Friend?", "Shared quests and groups will be removed for both of you. Are you sure?", async () => {
            ...
            groupsIOwn.forEach(item => {
                const originalGroup = mainItems.find(g => g.id === item.data.originalGroupId); // Changed from generalTaskGroups
                if (originalGroup) {
                    delete originalGroup.isShared;
                    delete originalGroup.sharedGroupId;
                    localStateChanged = true;
