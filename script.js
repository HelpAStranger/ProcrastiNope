...
const audioManager = {
    ...
    setVolume(volume) {
        ...
        if (this.isInitialized && this.masterGain) {
            ...
            // Cancel any ongoing ramps to prevent conflict
            this.masterGain.gain.cancelScheduledValues(now);
            // Anchor the ramp at the current value
    ...
    playSound(type) {
        ...
        // Check actual master gain value to avoid playing if silent (optimization)
        if (this.masterGain.gain.value < 0.01 && (!this.pendingVolume || this.pendingVolume < 0.01)) {
            // Optional: return; // But we might want to play silence for timing?
        }
